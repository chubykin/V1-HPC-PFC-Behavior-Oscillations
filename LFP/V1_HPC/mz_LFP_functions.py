...
def tf_cmw(ax,df_res, num_freq = 40, method = 3, show=True):
    ...
    base_idx = [0, int(0.4*2500)] # this is the baseline time window [0,0.4] but in samples
    min_freq = 2
    max_freq = 80
    num_frex = num_freq
    range_cycles = [3, 10]
...
#         #df_res = resampy.resample(df_res.values, Sampling_Rate, down_fs , axis=1)
    ...
    contour_levels = np.arange(0, 12, 0.25) #this changes the colorbar range!!!
    if show==True:
        ax.set_yscale('log')
        ...
        tf_plot = ax.contourf(time, frex, tf[0,:,:], 
                              contour_levels,  cmap = 'viridis' , extend = 'both') #where you can change the colors - viridis,afmhot
